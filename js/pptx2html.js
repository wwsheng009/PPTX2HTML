$(document).ready(function () {
	...
	if (window.Worker) {
		const fname = getURLParameter('file');
		if (!fname) {
			alert('Failed to get the pptx file')
			return
		}
		var $result = $("#result");
		var isDone = false;
		...
		fetch(fname)
			...
			.catch(error => {
				...
				$("#error_block").text("Error fetching the file");
			});
	...
	dependencies: [\
		...
		$("#to-reveal-btn").click(function () {
			if (localStorage) {
				...
				window.open("./reveal/play.html", "_blank");
			} else {
				alert("Browser don't support Web Storage!");
			}
		});
